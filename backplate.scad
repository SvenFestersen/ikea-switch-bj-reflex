...
module back_plate() {
    ...
    difference() {
        ...
        // remove part of the side alignment protrusion to form a hole
        translate([(baseplate_size - 5.0)/2, (baseplate_size - switch_size) / 2 - 2.5 - 3, -0.1])
        rounded_cube([5, 2.5, 10], r=0.4, $fn=50);
        translate([(baseplate_size - 5.0)/2, baseplate_size - (baseplate_size - switch_size) / 2 + 3, -0.1])
        rounded_cube([5, 2.5, 10], r=0.4, $fn=50);
        ...
        translate([(baseplate_size-switch_size)/2, (baseplate_size-switch_size)/2, 1.3])
        rounded_cube([switch_size, switch_size, 3.3], r=9.5, $fn=100);
        ...
        // emboss version
        translate([baseplate_size-10, baseplate_size-35, 1.1])
        rotate([0,0,90])
        version_string();
    }
}
