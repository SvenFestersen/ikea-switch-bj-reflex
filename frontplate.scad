...
module front_plate() {
    ...
    difference() {
        union() {
            ...
            cube([8, 5, 2.2]);
            // hinge base left
            translate([(w-8)/2, h-2.6, 0])
            cube([8, 2.6, 4]);
            translate([(w-8)/2, h-5, 0])
            cube([8, 5, 2.2]);
        }
        // IKEA switch cut-out
        ...
        // hinge cut-out right
        translate([(w-3.8)/2, -2.6, -0.1])
        rounded_cube([3.8, 7.3, 2.4], r=0.5);
        // hinge cut-out left
        translate([(w-3.8)/2, h-7.3+2.6, -0.1])
        rounded_cube([3.8, 7.3, 2.4], r=0.5);
        // cut-out in IKEA switch alignment wall to allow easy
        // insertion of the switch cover
        translate([(w-5)/2, (h-2*2.6) + (h - (h-2*2.6))/2, 1.6])
        rotate([90, 0 ,0])
        rounded_cube([5, 5, (h-2*2.6)], r=1, $fn=50);
        // screw holes
